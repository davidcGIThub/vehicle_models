...
class SplinePathManager():
    ...
    def get_current_path_control_points(self, position: np.ndarray, closest_point: np.ndarray):
        ...
        if self._path_index < self._num_paths-1:
            ...
            endpoint = self._spline_eval.get_end_point(control_points)
            is_closest_end = (np.linalg.norm(endpoint - closest_point) < 0.000001)
            if distance_to_end < self._tolerance or \
                distance_to_start_of_next < self._tolerance or \
                is_closest_end :
                self._path_index += 1
