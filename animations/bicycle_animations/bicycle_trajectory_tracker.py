...
class BicycleKinematicController:
    ...
    def mpc_control_accel_input(self, states, trajectory_states):
        ...
        # Clip and Adjust velocity dot command 
        # vel = np.sqrt(x_dot**2 + y_dot)
        # vel_min = np.min((self._vel_turn, vel_des))
        # if vel < vel_min:
        #     vel_dot_min = (vel_min - vel)*self._k_vel
        #     vel_dot_des = np.max((vel_dot_des, vel_dot_min))
    ...
    def mpc_control_velocity_input(self, states, trajectory_states):
        ...
        #desired trajectory states
        x_path = trajectory_states.item(0,0)
        y_path = trajectory_states.item(0,1)
        x_dot_path = trajectory_states.item(1,0)
        y_dot_path = trajectory_states.item(1,1)
